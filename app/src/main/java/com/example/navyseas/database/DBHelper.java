...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void test_pay(Payment p) {
        if (p.getAmount() > 0) {
            SQLiteDatabase db = this.getWritableDatabase();
    ...
    public void test_deletePayment(Payment payment) {
        SQLiteDatabase db = this.getWritableDatabase();

        String query = "DELETE FROM " + PAYMENT_TABLE +
                " WHERE " + PAYMENT_COLUMN_DATE + " = '" + payment.getDate()+"'";
